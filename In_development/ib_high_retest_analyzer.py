...
def load_session_summary_data(db_path, table_name):
    ...
    try:
        ...
        # Select necessary columns including IB_High
        query = f"SELECT * FROM {table_name}"
        df = pd.read_sql_query(query, conn)
        conn.close()

        required_columns = ['Date', 'Sessions', 'SessionStart', 'SessionEnd',
                            'SessionLow', 'SessionHigh', 'SessionClose', 'IB_High'] # Changed IB_Low to IB_High
        ...
        # Ensure IB_High is numeric
        df['IB_High'] = pd.to_numeric(df['IB_High'], errors='coerce')
...
def analyze_ib_high_retest(df, lookahead=LOOKAHEAD_PERIOD):
    """Analyzes the probability of IB_High retest after NY close above IB."""
    if df is None:
    ...
    # Identify trigger sessions (NY Close > IB_High)
    ny_sessions.dropna(subset=['IB_High', 'SessionClose'], inplace=True) # Drop rows where IB_High or SessionClose is NaN
    trigger_sessions = ny_sessions[ny_sessions['SessionClose'] > ny_sessions['IB_High']]

    total_triggers = len(trigger_sessions)
    if total_triggers == 0:
        print("No trigger sessions (New York close > IB_High) found.")
        return None

    print(f"Found {total_triggers} trigger sessions (NY Close > IB_High).")
    ...
    for index, trigger_row in trigger_sessions.iterrows():
        trigger_ib_high = trigger_row['IB_High'] # Changed to IB_High
        # Find original index in the full dataframe
        # Use boolean indexing which is generally safer than assuming direct iloc relationship after filtering
        trigger_df_indices = df.index[(df['SessionStart'] == trigger_row['SessionStart']) & (df['Sessions'] == 'NewYork')].tolist()
        if not trigger_df_indices:
            print(f"Warning: Could not find original index for trigger session: {trigger_row['SessionStart']}")
            continue
        trigger_df_index = trigger_df_indices[0]

        retest_found = False
        for k in range(1, lookahead + 1):
            next_session_index = trigger_df_index + k
            if next_session_index >= len(df):
                break # Reached end of data for this trigger

            next_session_row = df.iloc[next_session_index]
            session_low = next_session_row['SessionLow']
            # session_high = next_session_row['SessionHigh'] # Not needed for this condition

            # Check for retest condition: Did the low of the subsequent session touch or break the trigger's IB_High?
            if session_low <= trigger_ib_high:
                retest_counts[k] += 1
                retest_found = True
    ...
    print("\n--- IB_High Retest Probability Analysis ---") # Updated Title
    print(f"Trigger: New York session closes ABOVE IB_High") # Updated Trigger Desc
...
if __name__ == "__main__":
    print("Starting IB High Retest Analysis...") # Updated Start Message
    session_df = load_session_summary_data(DATABASE_PATH, TABLE_NAME)

    if session_df is not None:
        results = analyze_ib_high_retest(session_df, lookahead=LOOKAHEAD_PERIOD) # Changed function call
        if results:
            print("\nAnalysis complete.")
        else:
            print("\nAnalysis could not be completed (e.g., no trigger events found).")
    else:
